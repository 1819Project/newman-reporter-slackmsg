...
function slackMessage(stats, timings, failures, executions, maxMessageSize, collection, collectionName, environment, channel, reportingUrl, limitFailures) {
    ...
    "attachments": [
        {
            ...
            "fields": [
                ${limitFailures > 0 ? failMessage(parsedFailures.splice(0, limitFailures)) : failMessage(parsedFailures,collectionName)}
            ],
            "footer": "Newman Test",
...
function failMessage(parsedFailures,collectionName) {
    return parsedFailures.map((failure) => {
        return `
        {
            "title": "*\`${collectionName} - ${failure.name}\`*",
            "short": false
        },
